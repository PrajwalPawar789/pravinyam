...
const ProblemSolving = () => {
  ...
  const [solverQues, setSolverQues] = useState([]);
  ...
  function getQuestions() {
    fetch("http://localhost:8090/api/exercises?module=solver")
      .then((result => {
  ...
  return (
    <Container maxWidth="100%" sx={{
          ...
          return (
            <Stack direction="row" alignItems="center" key={label} sx={{
              ...
              <Typography sx={{
                ...
                {label === 'All exercises' ? solverQues.length :
                  0}
              </Typography>
            </Stack>
          );
      ...
      <Box sx={{
        ...
        <Grid container>
          {
            solverQues.filter(item => item.title.match(new RegExp(query, 'i')))
              .map((prop) => {
                return (

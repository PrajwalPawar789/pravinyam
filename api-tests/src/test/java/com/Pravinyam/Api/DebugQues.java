...
public class DebugQues {

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		RestAssured.baseURI = "http://localhost:8090/api";

		Response response = given().when().get("exercises?module=debug").then().log().all().extract().response();
...
//		Assert.assertEquals(jsp.getList("language").get(0), "java");
		Assert.assertEquals(jsp.getList("module").get(0), "debug");
		
		ArrayList<Integer> ids = given().contentType(ContentType.JSON)
				.get("exercises?module=debug").then().extract().path("exid");
